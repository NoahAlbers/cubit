...
module.exports=async({request,db,administrator,member})=>{
  ...
  const publicSettings=await ok('/api/organization/public',null,'GET',null);assert.deepEqual(Object.keys(publicSettings).sort(),['currency','name','supportEmail','timezone']);
  ...
  let settings=await ok('/api/organization/settings',{name:'Synthetic Community Lab',supportEmail:'help@community.example',timezone:'Asia/Tokyo',currency:'EUR',revision:initial.settings.revision},'PUT');
  ...
  assert.equal(settings.timezone,'Asia/Tokyo');assert.equal(settings.currency,'EUR');
  assert.equal((await request('/api/organization/settings',{name:settings.name,supportEmail:settings.supportEmail,currency:'BAD',revision:settings.revision},'PUT',adminToken)).status,400);
  assert.equal((await request('/api/organization/settings',{name:settings.name,supportEmail:settings.supportEmail,timezone:'Invalid/Zone',revision:settings.revision},'PUT',adminToken)).status,400);
  ...
  await ok('/api/organization/settings',{name:initial.settings.name,supportEmail:initial.settings.supportEmail,timezone:initial.settings.timezone,currency:initial.settings.currency,revision:settings.revision},'PUT');
