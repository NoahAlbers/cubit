...
module.exports=async({request,db,administrator,member})=>{
  ...
  const publicSettings=await ok('/api/organization/public',null,'GET',null);assert.deepEqual(Object.keys(publicSettings).sort(),['name','supportEmail','timezone']);
  ...
  let settings=await ok('/api/organization/settings',{name:'Synthetic Community Lab',supportEmail:'help@community.example',timezone:'Asia/Tokyo',revision:initial.settings.revision},'PUT');
  ...
  await ok('/api/organization/settings',{name:initial.settings.name,supportEmail:initial.settings.supportEmail,timezone:initial.settings.timezone,revision:settings.revision},'PUT');
