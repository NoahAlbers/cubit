...
(async () => {
  ...
  const auditId=randomUUID(), rolledBackId=randomUUID();
  await db.query("INSERT INTO operations_audit(id,memberId,kind,author,detail) VALUES (?,?,'Fixture updated','Migration test','{}')",[auditId,memberId]);
  const original=(await db.query('SELECT payload FROM audit_outbox WHERE auditId=?',[auditId]))[0].payload;
  assert.equal(JSON.parse(original).memberEmail,'migration@test.example');
  await assert.rejects(db.transaction(async manager=>{
    await manager.query("INSERT INTO operations_audit(id,memberId,kind,author,detail) VALUES (?,?,'Rollback','Migration test','{}')",[rolledBackId,memberId]);
    throw Error('rollback fixture');
  }),/rollback fixture/);
  assert.equal((await db.query('SELECT id FROM audit_outbox WHERE auditId=?',[rolledBackId])).length,0,'Rolled-back actions must not be delivered');
  await db.query("UPDATE member SET email='changed@test.example' WHERE id=?",[memberId]);
  assert.equal((await db.query('SELECT payload FROM audit_outbox WHERE auditId=?',[auditId]))[0].payload,original,'Outbox captures the identity at event time');
  // Reproduce supported pre-migration differences without ever using a real DB.
  await db.query('DROP TABLE cubit_migrations');
  await db.query('DROP TRIGGER IF EXISTS cubit_audit_outbox');
  await db.query('DROP TABLE audit_outbox');
  ...
  await db.query('DROP TABLE health_sample');
  ...
  await db.runMigrations({transaction:'none'});
  assert.equal((await db.query('SELECT auditId FROM audit_outbox WHERE auditId=?',[auditId])).length,1,'Legacy audit backfill');
