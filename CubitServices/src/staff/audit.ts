...
export function snapshot(value: any, fields: string[]) {
  return value ? Object.fromEntries(fields.map((k) => [k, value[k] ?? null])) : null;
}
...
export async function recordAudit(
  ...
  return manager.save(
    OperationsAudit,
    manager.create(OperationsAudit, {
      ...
      detail: JSON.stringify({
        ...
        actorType: input.actorType || (actor ? 'staff' : 'system'),
...
export function auditDetail(row: OperationsAudit) {
  ...
  try {
    ...
    /* Older history may contain plain text rather than JSON. */
  }
  ...
  return {
    ...
    actorType:
      detail?.actorType ||
      (row.author === 'Automation'
        ? 'system'
        : row.kind.startsWith('Member updated') || row.kind === 'Demo waiver signed'
          ? 'member'
          : 'staff'),
    legacy: detail?.legacy === true || detail?.version !== 1,
    entityId: detail?.entityId || detail?.planId || null,
