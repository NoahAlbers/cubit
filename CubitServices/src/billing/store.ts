...
export async function postCharges(
  ...
  for (const p of plans) {
    if (p.billingRate === null || p.billingRate === undefined) {
      p.billingRate = Number(p.plan.monthlyCost);
      p.billingName = p.plan.name;
      await manager.update(MemberPlan, p.id, {
        billingRate: p.billingRate,
        billingName: p.billingName,
      });
    }
    const stop = [asOf, p.finalBillingDate, p.endDate && day(p.endDate)]
      .filter(Boolean)
      .sort()[0] as string;
    for (let n = 0; ; n++) {
      ...
      if (!existing.some((c) => c.memberPlanId === p.id && c.dueDate === dueDate)) {
        await manager.save(
          BillingCharge,
          manager.create(BillingCharge, {
            memberId,
            memberPlanId: p.id,
            planName: p.billingName,
            dueDate,
            amount: p.billingRate!,
            voided: false,
          }),
        );
        count++;
...
export async function readBilling(
  ...
  const [plans, payments, charges, adjustments] = await Promise.all([
    ...
    manager.find(Transaction, { where: { memberId } }),
    manager.find(BillingCharge, { where: { memberId } }),
    manager.find(ChargeAdjustment, { where: { memberId }, order: { createdAt: 'DESC' } }),
  ]);
  return {
    plans,
    payments,
    records: charges,
    adjustments,
    ledger: postedLedger(charges, payments, adjustments, asOf),
  };
}
...
export async function refreshAccess(manager: EntityManager, member: Member, author = 'Automation') {
  ...
  if (member.billingSuspended !== decision.suspended)
    await recordAudit(manager, {
      memberId: member.id,
      kind: decision.suspended ? 'Billing access suspended' : 'Billing access restored',
      author,
      actorType: 'system',
      before: { billingSuspended: member.billingSuspended },
      after: { billingSuspended: decision.suspended },
      reason: decision.reason,
    });
  await manager.update(Member, member.id, {
    status: decision.status,
    balance: state.ledger.balance,
    billingSuspended: decision.suspended,
...
export async function initializeBilling() {
  await AppDataSource.transaction(async (manager) => {
    ...
    if (settings) return;
    settings = manager.create(OperationsSettings, {
      id: 'default',
      graceDays: 60,
      dailyEnabled: true,
    });
    ...
    for (const member of members) {
      ...
      const plans = await manager.find(MemberPlan, {
      ...
      if (before.balance !== after.balance || before.pastDue !== after.pastDue)
        throw Error('Billing migration reconciliation failed');
    }
    settings.migrationSummary = JSON.stringify({
