...
export function directoryBilling(
  member: any,
  plans: any[],
  records: any[],
  payments: any[],
  ...
  let needsRefresh =
    Number(member.balance) !== ledger.balance ||
    member.status !== decision.status ||
    Boolean(member.billingSuspended) !== decision.suspended ||
    String(member.statusReason || '') !== String(decision.reason || '');
