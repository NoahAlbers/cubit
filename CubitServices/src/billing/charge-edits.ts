...
function money(value: unknown, allowZero = false) {
  if (
    ...
    fail('Enter a valid amount with at most two decimal places.');
...
export async function addCharge(
  ...
  if (duplicate) {
    if (
      ...
      Math.round(Number(duplicate.amount) * 100) !== amount ||
      duplicate.dueDate !== input.date ||
      duplicate.planName !== name
    )
      fail('This request ID was already used for a different charge.', 409);
    return duplicate;
  }
  const charge = await manager.save(
    BillingCharge,
    manager.create(BillingCharge, {
      memberId,
      memberPlanId: `manual:${key}`,
      planName: name,
      dueDate: input.date,
      amount: amount / 100,
      requestKey: key,
  ...
  await recordAudit(manager, {
    ...
    author,
    entityId: charge.id,
    after: { amount: charge.amount, dueDate: charge.dueDate, description: charge.planName },
  });
...
export async function editCharge(
  ...
  const amount = money(input.amount, true),
    expected = money(input.expectedAmount, true);
  ...
  if (duplicate) {
    if (
      duplicate.chargeId !== chargeId ||
      Math.round(Number(duplicate.credit) * 100) !== expected - amount
    )
      fail('This request ID was already used for another change.', 409);
    return duplicate;
  }
  ...
  if (Math.round(current.amount * 100) !== expected)
    fail('The charge changed. Reopen it before editing.', 409);
  ...
  await recordAudit(manager, {
    memberId: member.id,
    kind: 'Charge corrected',
    author,
    entityId: chargeId,
    before: { amount: expected / 100 },
    after: { amount: amount / 100 },
    reason,
  });
  await refreshAccess(manager, member, author);
  return saved;
