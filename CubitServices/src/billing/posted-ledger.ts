...
export function postedLedger(
  ...
  for (const c of charges) {
    const allocated = Math.min(available, c.amount);
    c.outstanding -= allocated;
    available -= allocated;
  }
  ...
  return {
    ...
    pastDue: late.reduce((n, c) => n + c.outstanding, 0) / 100,
...
export function accessDecision(
  member: any,
  plans: any[],
  ledger: ReturnType<typeof postedLedger>,
  graceDays: number,
) {
  const current = plans.some(
    (p) =>
      day(p.startDate) <= ledger.asOf &&
      (!p.endDate || day(p.endDate) >= ledger.asOf) &&
      (!p.finalBillingDate || p.finalBillingDate >= ledger.asOf),
  );
  const ended = plans.some(
    (p) =>
      (p.finalBillingDate && p.finalBillingDate < ledger.asOf) ||
      (p.endDate && day(p.endDate) < ledger.asOf),
  );
  ...
  const status = !current
    ? ended
      ? 'Canceled'
      : 'Inactive'
    : member.accessHold || suspended
      ? 'Inactive'
      : 'Active';
  return {
    status,
    suspended,
    reason: member.accessHold
      ? member.accessHoldReason
      : !current
        ? 'No current membership'
        : suspended
          ? 'Outside billing grace period'
          : ledger.pastDue
            ? 'Within billing grace period'
            : 'Current',
  };
}
