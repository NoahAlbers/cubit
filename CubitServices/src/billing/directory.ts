...
export function matchesSearch(member: any, query: string, field = 'all') {
  const fields = field === 'all' ? ['firstName', 'lastName', 'email', 'paypalEmail', 'phone'] : [field]
  ...
  return normalized.split(/\s+/).every(token => fields.some(f => String(member[f] || '').toLowerCase().includes(token)))
}
...
export async function directoryRows() {
  ...
  const [members, logs, keys, plans, payments, charges, adjustments, settings] = await Promise.all([
    ...
    AppDataSource.manager.createQueryBuilder(AccessLog, 'log')
      .select('log.memberId', 'memberId').addSelect('MAX(log.timestamp)', 'lastAttempt')
      .addSelect('MAX(CASE WHEN log.accessGranted = 1 AND log.timestamp <= :now THEN log.timestamp ELSE NULL END)', 'lastKeyUsage')
      .addSelect('MAX(CASE WHEN log.accessGranted = 1 AND log.timestamp >= :since AND log.timestamp <= :now THEN 1 ELSE 0 END)', 'checkedIn30Days')
  ...
  const asOf = day(now)
  const groupedPlans = byMember(plans), groupedPayments = byMember(payments), groupedCharges = byMember(charges), groupedAdjustments = byMember(adjustments)
  ...
  for (const member of members) {
    const snapshot = directoryBilling(member, groupedPlans.get(member.id) || [], groupedCharges.get(member.id) || [],
      groupedPayments.get(member.id) || [], groupedAdjustments.get(member.id) || [], settings.graceDays, asOf)
    const { plans: memberPlans, ledger, status, reason, suspended } = snapshot.needsRefresh ? await ensureBilling(member.id) : snapshot
    ...
    result.push({ ...safe, status, statusReason: reason, billingSuspended: suspended, enabledKeys, accessAllowed: status === 'Active' && enabledKeys > 0,
...
export function filterDirectory(rows: any[], query: Record<string, any>) {
  ...
  if (query.overdue === 'true' && query.includeEnded !== 'true') result = result.filter(m => m.status !== 'Canceled')
  ...
  if (query.activity === '30' || query.activity === '90') result = result.filter(m => m.lastKeyUsage &&
    today - Date.parse(day(new Date(m.lastKeyUsage))) <= Number(query.activity) * 86400000)
  if (query.activity === 'stale') result = result.filter(m => m.lastKeyUsage &&
    today - Date.parse(day(new Date(m.lastKeyUsage))) > 90 * 86400000)
  const sort = ['name','contact','status','plan','access','recent','balance','amount','oldest'].includes(query.sort) ? query.sort : 'name'
  const order = ['asc','desc'].includes(query.order) ? query.order : ['recent','balance','amount','oldest'].includes(sort) ? 'desc' : 'asc'
  const text = (a: any,b: any) => String(a || '').localeCompare(String(b || ''),'en',{sensitivity:'base',numeric:true})
  const name = (a: any,b: any) => text(a.lastName,b.lastName) || text(a.firstName,b.firstName) || text(a.id,b.id)
  result.sort((a, b) => {
    ...
    if (sort==='contact' && !a.email?.trim() !== !b.email?.trim()) return a.email?.trim() ? -1 : 1
    let diff=0
    ...
    if(sort==='recent') diff=(a.lastKeyUsage ? +new Date(a.lastKeyUsage) : 0)-(b.lastKeyUsage ? +new Date(b.lastKeyUsage) : 0)
  ...
  return { rows: result.slice((page - 1) * pageSize, page * pageSize), total: result.length, page, pages, pageSize, sort, order,
    summary: { total: rows.length, active: rows.filter(m => m.status === 'Active').length,
      ...
      oldestDays: Math.max(0, ...result.map(m => m.daysPastDue)) }, asOf: day(new Date()) }
}
