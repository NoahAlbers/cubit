...
export async function automationPlan(manager: EntityManager, settings: OperationsSettings, apply: boolean) {
  const today = day(new Date()), actions: any[] = []
  let newCharges = 0
  ...
  for (const row of members) {
    ...
    const scheduled = billingLedger(before.plans.map(p => ({ ...p, plan: {
      name: p.billingName || p.plan.name, monthlyCost: p.billingRate ?? p.plan.monthlyCost } })), []).charges
    const missing = scheduled.filter(c => !before.records.some(r => r.memberPlanId === c.planId && r.dueDate === c.dueDate))
    newCharges += missing.length
    const previewLedger = postedLedger([...before.records, ...missing.map(c => ({ ...c, id: `${c.planId}:${c.dueDate}`, memberPlanId: c.planId }))], before.payments, before.adjustments)
    ...
    if (member.status !== decision.status || member.billingSuspended !== decision.suspended)
      actions.push({ memberId: member.id, name: `${member.firstName} ${member.lastName}`, from: member.status || 'Inactive', to: decision.status, reason: decision.reason })
    if (apply) { await postCharges(manager, member.id); await refreshAccess(manager, member) }
  }
...
export async function runAutomation(preview: boolean, trigger: string, daily = false) {
  ...
  try {
    ...
    if (!preview) await AppDataSource.manager.save(AutomationRun, AppDataSource.manager.create(AutomationRun, {
      id: `${id}:failed:${randomUUID()}`, status: 'Failed', trigger, summary: JSON.stringify({ error: err.message }) }))
    throw err
  }
}
...
export function startAutomationScheduler() {
  ...
  const tick = async () => {
    ...
    busy = true
    try { await runAutomation(false, 'Daily schedule', true) }
    catch (err: any) { console.error('Automation run failed:', err.message) }
    finally { busy = false }
  }
