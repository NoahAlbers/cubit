...
export function catalogValues(body: any) {
  if (typeof body?.name !== 'string' || !body.name.trim() || body.name.trim().length > 100)
    fail('Enter a plan name (1–100 characters).');
  const monthlyCost = body.monthlyCost;
  if (
    typeof monthlyCost !== 'number' ||
    !Number.isFinite(monthlyCost) ||
    monthlyCost < 0 ||
    monthlyCost > 99999999 ||
    Math.abs(monthlyCost * 100 - Math.round(monthlyCost * 100)) > 0.00001
  )
...
export async function saveCatalogPlan(id: string, body: any, author: string, create = false) {
  const values = catalogValues(body);
  catalogId(id);
  return AppDataSource.transaction(async (manager) => {
    ...
    if (create) {
      if (plan) {
        if (
          ...
          Number(plan.monthlyCost) === values.monthlyCost &&
          plan.available === values.available
        )
          return plan;
    ...
    if (body.revision !== plan.revision)
      fail('Someone changed this plan. Reload it before saving.', 409);
    const before = {
      name: plan.name,
      monthlyCost: Number(plan.monthlyCost),
    ...
    await recordAudit(manager, {
      kind:
        before.available && !values.available
          ? 'Plan retired'
          : !before.available && values.available
            ? 'Plan restored'
            : 'Plan updated',
...
async function freezeAssignments(manager: EntityManager, plan: Plan) {
  await manager
    .createQueryBuilder()
    .update(MemberPlan)
    .set({ billingRate: Number(plan.monthlyCost) })
    .where('planId = :id AND billingRate IS NULL', { id: plan.id })
    .execute();
  await manager
    .createQueryBuilder()
    .update(MemberPlan)
    .set({ billingName: plan.name })
    .where('planId = :id AND billingName IS NULL', { id: plan.id })
    .execute();
}
