...
export class Member {
  ...
  public async checkForDuplicateEmail(email: string): Promise<boolean> {
    const [, count] = await AppDataSource.manager.findAndCountBy(Member, {
      email: email,
    });
    return Promise.resolve(count > 0);
  }

  public async GetMemberByEmailAndPass(email: string, password: string): Promise<Member> {
    ...
    const matchesByEmail = await AppDataSource.manager.find(Member, {
      where: {
        email: Raw(
          (alias) =>
            `LOWER(REGEXP_REPLACE(${alias}, '^[[:space:]]+|[[:space:]]+$', '')) = :loginEmail`,
    ...
    const hasPassword =
      !!member && !member.loginDisabled && /^\$2[aby]\$\d{2}\$/.test(member.password || '');
    ...
    await new Promise((resolve) =>
      setTimeout(resolve, Math.max(0, 400 - (performance.now() - started))),
    );
  ...
  public async GetMemberByPaypalEmail(email: string): Promise<Member> {
    return AppDataSource.manager.findOneByOrFail(Member, { paypalEmail: email }).then(
      async (member) => {
  ...
  public async getCurrentPlan(memberId: string): Promise<MemberPlan | null> {
    ...
    return (
      plans
        .filter(
          (p) =>
            day(p.startDate) <= today &&
            (!p.endDate || day(p.endDate) >= today) &&
