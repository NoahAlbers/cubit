...
export function docusealConfig() {
  ...
  if (
    ...
    fail('Invalid DocuSeal public URL.', 503);
...
async function api(method: 'GET' | 'POST', path: string, data?: any) {
  ...
  if (!config.enabled)
    fail('DocuSeal is not connected. Use the local demo or configure a sandbox API key.', 503);
  try {
    return (
      await axios({
        method,
    ...
    fail('DocuSeal could not be reached. Please try checking the signing status again.', 502);
  }
}
export async function validateTemplate(id: number, role: string) {
  ...
  if (
    ...
    template.submitters[0].name !== role
  )
    fail('Choose a DocuSeal template with exactly one signer and a matching signer role.');
  if (!template.fields?.some((f: any) => f.type === 'signature' && f.required))
    fail('The template needs a required signature field.');
  if (template.documents?.length !== 1)
    fail('Choose a template containing one PDF. Combine all pages into one PDF first.');
...
export async function createSigning(version: WaiverVersion, signature: WaiverSignature) {
  ...
  if (version.providerFingerprint !== template.fingerprint)
    fail(
      'This DocuSeal template changed after publication. Staff must publish a new waiver version.',
      409,
    );
  const result = await api('POST', '/submissions', {
    template_id: version.docusealTemplateId,
    send_email: false,
    send_sms: false,
    submitters: [
      {
        ...
        role: version.signerRole,
        external_id: signature.id,
        send_email: false,
        send_sms: false,
      },
    ],
  ...
  if (
    ...
    result.length !== 1 ||
    !Number.isSafeInteger(result[0].submission_id) ||
    !Number.isSafeInteger(result[0].id) ||
    !/^[a-zA-Z0-9]+$/.test(result[0].slug)
  )
...
export async function retrieveSigning(signature: WaiverSignature) {
  ...
  if (!signature.submissionId) {
    ...
    if (!Array.isArray(found.data) || found.data.length !== 1)
      fail(
        'Signing request needs staff review in DocuSeal. A duplicate request has not been created.',
        409,
      );
...
export function verifiedCompletion(data: any, signature: WaiverSignature, version: WaiverVersion) {
  ...
  if (
    ...
    data.template?.id !== version.docusealTemplateId ||
    !signer ||
    signer.email?.toLowerCase() !== signature.signerEmail.toLowerCase() ||
    (signature.submissionId && signature.submissionId !== data.id) ||
    !Number.isSafeInteger(signer.id) ||
    !/^[a-zA-Z0-9]+$/.test(signer.slug)
  )
    fail('DocuSeal signing details do not match this waiver.', 502);
  const completed =
    data.status === 'completed' &&
    signer.status === 'completed' &&
    !!signer.completed_at &&
    Number.isFinite(Date.parse(signer.completed_at));
  const url = data.documents?.[0]?.url;
  let documentUrl: string | undefined;
  if (completed && typeof url === 'string') {
    const parsed = new URL(url);
    if (
      parsed.origin === new URL(docusealConfig().publicUrl).origin &&
      !parsed.username &&
      !parsed.password
    )
      documentUrl = url;
  ...
  return {
    ...
    status: completed
      ? 'Signed'
      : ['declined', 'expired'].includes(data.status)
        ? 'Needs review'
        : 'Pending',
    completedAt: completed ? new Date(signer.completed_at) : null,
...
export async function downloadSigningFile(url: string) {
  ...
  if (
    !config.enabled ||
    parsed.origin !== new URL(config.publicUrl).origin ||
    parsed.username ||
    parsed.password ||
    !/^\/(rails\/active_storage\/|file\/|blobs_proxy\/)/.test(parsed.pathname)
  )
    fail('DocuSeal returned an unexpected document location.', 502);
  const base = new URL(config.apiUrl);
  const target =
    base.hostname === '127.0.0.1' ? base.origin + parsed.pathname + parsed.search : parsed.href;
  try {
    ...
    fail('The signed file could not be archived yet. Check signing status again.', 502);
  }
}
