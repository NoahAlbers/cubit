...
export function startWaiverReconciliation(){
  ...
  const tick=async()=>{
    ...
    running=true
    try{
      const pending=await AppDataSource.manager.createQueryBuilder(WaiverSignature,'s')
        .where("s.provider='docuseal' AND s.status IN ('Pending','Preparing','Needs review')")
        .orderBy('s.createdAt','ASC').getMany()
      for(const s of pending){
        if(stopped)break
        try{await syncSigning(s.memberId,s.id)}catch{console.warn('Waiver archival pending; will retry:',s.id)}
      }
    }catch{console.warn('Waiver reconciliation unavailable; will retry.')}
