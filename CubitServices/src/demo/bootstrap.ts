...
export async function initializeHostedDemo(password: string) {
  if (
    ...
    localConfig.database !== 'cubit_demo' ||
    localConfig.username !== 'cubit_demo'
  )
    throw Error('Demo initialization requires the separate synthetic database.');
  if (!password || password.length < 10)
    throw Error('Supply a demo password of at least 10 characters.');
  await AppDataSource.initialize();
  try {
    await assertSchemaReady(AppDataSource);
    if (await AppDataSource.manager.count(Member))
      throw Error(
        'Demo initialization requires an empty member table; existing data was not changed.',
      );
    await seedLocalData();
    await seedScenarios();
    ...
    await AppDataSource.transaction(async (manager) => {
      ...
      await manager.update(Member, demoMemberId, {
        email: demoEmail,
        paypalEmail: demoEmail,
        firstName: 'Demo',
    ...
    await AppDataSource.query(
      "INSERT INTO cubit_demo_manifest (id, complete) VALUES ('synthetic-v1', TRUE)",
    );
...
if (require.main === module)
  initializeHostedDemo(process.env.DEMO_LOGIN_PASSWORD || '').catch((error) => {
