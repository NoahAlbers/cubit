...
const fixtureId = (kind: number, n: number) =>
  ...
  await AppDataSource.transaction(async (manager) => {
    ...
    for (let i = 0; i < 72; i++) {
      ...
      const firstName = firstNames[i % firstNames.length],
        lastName = lastNames[Math.floor(i / firstNames.length)];
      const member = manager.create(Member, {
        id: fixtureId(5, i + 1),
        firstName,
        lastName,
        email: `${firstName}.${lastName}@example.test`.toLowerCase(),
        paypalEmail: `billing.${i + 1}@example.test`,
        phone: `(202) 555-${String(100 + i).padStart(4, '0')}`,
        emergencyContact: `Demo contact ${i + 1}`,
        emergencyEmail: `contact.${i + 1}@example.test`,
      ...
      const membership = manager.create(MemberPlan, {
        ...
        plan,
        startDate:
          scenario === 7 ? new Date(today.getFullYear(), today.getMonth() + 1, 1, 12) : start,
        paypalSubscriptionId: `SYNTHETIC-${i + 1}`,
      ...
      let paymentBudget = Math.max(
        0,
        charges.length * Number(plan.monthlyCost) - unpaidMonths * Number(plan.monthlyCost),
      );
      ...
      for (const charge of charges) {
        ...
        payments.push(
          manager.create(Transaction, {
            memberId: member.id,
            amount,
            transactionDate: new Date(`${charge.dueDate}T12:00:00`),
            method: 'Local demo',
            description: 'Synthetic membership payment',
            confirmation: `DEMO-${i + 1}-${charge.dueDate}`,
          }),
        );
      ...
      if (paymentBudget > 0)
        payments.push(
          manager.create(Transaction, {
            ...
            method: 'Local demo',
            description: 'Synthetic advance payment',
          }),
        );
      ...
      await manager.save(
        MemberKey,
        manager.create(MemberKey, {
          id: fixtureId(7, i + 1),
          memberId: member.id,
          serialNumber: `DEMO${String(i + 101).padStart(8, '0')}`,
          status: 'Active',
        }),
      );
      if (canceled)
        await manager.save(
          ...
          manager.create(BillingChange, {
            ...
            previousDate: null,
            finalBillingDate: cutoff,
            reason: 'Synthetic cancellation example',
      ...
      if (daysAgo >= 0 && scenario !== 6 && scenario !== 7) {
        for (let visit = 0; visit < 6; visit++) {
          const timestamp = new Date(
            today.getFullYear(),
            today.getMonth(),
            today.getDate() - daysAgo - visit * 12,
            14 + (i % 5),
          );
          if (canceled && day(timestamp) > cutoff) continue;
          await manager.save(
            AccessLog,
            manager.create(AccessLog, {
              member,
              timestamp,
              accessGranted: true,
              message: 'Synthetic workshop sign-in',
            }),
          );
        }
      }
      if (i % 4 === 0)
        await manager.save(
          ...
          manager.create(AccessLog, {
            ...
            timestamp: today,
            accessGranted: false,
            message: 'Synthetic denied key attempt',
          }),
        );
    }
  });
  console.log(
    'Added Cubit fixture set v1: 72 synthetic members, payments, cancellations and access logs.',
  );
}
