...
export function readLocalConfig(env: NodeJS.ProcessEnv) {
  ...
  if (
    !['127.0.0.1', 'localhost', 'mysql'].includes(host) ||
    !['TonicLocalDev', 'TonicLocalReview'].includes(database) ||
    username !== 'tonic_local' ||
  ...
  return {
    ...
    host,
    database: process.platform === 'win32' ? database.toLowerCase() : database,
    username,
...
export function readHostedReviewConfig(env: NodeJS.ProcessEnv) {
  if (
    env.CUBIT_MODE !== 'hosted-review' ||
    env.LOCAL_DEVELOPMENT === 'true' ||
    env.DATABASE_URI !== '127.0.0.1' ||
    env.DATABASE_NAME !== 'cubit_review' ||
    env.DATABASE_USERNAME !== 'cubit_app' ||
    env.HOST !== '127.0.0.1' ||
    !env.DATABASE_PASSWORD ||
    !env.JWT_SECRET ||
    env.JWT_SECRET.length < 48 ||
    (env.DOCUSEAL_ENABLED === 'true' &&
      (env.DOCUSEAL_API_URL !== 'http://127.0.0.1:3000/api' ||
        !env.DOCUSEAL_API_KEY ||
        !env.DOCUSEAL_PUBLIC_URL?.startsWith('https://')))
  ) {
    throw new Error('Invalid hosted review configuration.');
  }
  const port = Number(env.PORT || 5001),
    databasePort = Number(env.DATABASE_PORT || 3306);
  if (![port, databasePort].every((p) => Number.isInteger(p) && p > 0 && p < 65536))
  ...
  return {
    ...
    jwtSecret: env.JWT_SECRET,
    dataMode: 'imported',
    runtimeMode: 'hosted-review' as const,
  };
}

export function readHostedDemoConfig(env: NodeJS.ProcessEnv) {
  if (
    env.CUBIT_MODE !== 'hosted-demo' ||
    env.LOCAL_DEVELOPMENT === 'true' ||
    env.DATABASE_URI !== '127.0.0.1' ||
    env.DATABASE_NAME !== 'cubit_demo' ||
    env.DATABASE_USERNAME !== 'cubit_demo' ||
    env.HOST !== '127.0.0.1' ||
    !env.DATABASE_PASSWORD ||
    !env.JWT_SECRET ||
    env.JWT_SECRET.length < 48 ||
    ...
    throw new Error('Invalid hosted demo configuration.');
  ...
  return {
    ...
    jwtSecret: env.JWT_SECRET,
    dataMode: 'demo',
    runtimeMode: 'hosted-demo' as const,
  };
}

export const localConfig =
  process.env.CUBIT_MODE === 'hosted-review'
    ? readHostedReviewConfig(process.env)
    : process.env.CUBIT_MODE === 'hosted-demo'
      ? readHostedDemoConfig(process.env)
      : readLocalConfig(process.env);
// The captured Tonic server uses UTC calendar days. Preserve its billing dates.
if (localConfig.dataMode === 'imported' || localConfig.runtimeMode === 'hosted-demo')
  process.env.TZ = 'UTC';
