...
export function createRememberedGreeting(options: Options) {
  ...
  const key = createHmac('sha256', options.secret).update(`cubit:greeting:v1:${options.workspace}`).digest()
  ...
  function remember(res: Response, member: KnownMember) {
    ...
    const payload = JSON.stringify({
      id: member.id, email: emailDigest(member.email.trim().toLowerCase()).toString('hex'), expires: now() + LIFETIME,
    })
    const encrypted = Buffer.concat([cipher.update(payload, 'utf8'), cipher.final()])
    const cookie = Buffer.concat([iv, cipher.getAuthTag(), encrypted]).toString('base64url')
    res.cookie(COOKIE, cookie, {
      httpOnly: true, secure: options.secure, sameSite: 'strict',
      path: '/login/greeting', maxAge: LIFETIME,
    })
  }
  ...
  const greet: RequestHandler = async (req, res) => {
    ...
    try {
      ...
      const claims = JSON.parse(Buffer.concat([decipher.update(bytes.subarray(28)), decipher.final()]).toString('utf8'))
      if (typeof claims.id !== 'string' || claims.id.length > 64 ||
          !Number.isFinite(claims.expires) || claims.expires <= time || claims.expires > time + LIFETIME ||
          typeof claims.email !== 'string' || !/^[a-f0-9]{64}$/.test(claims.email) ||
          !timingSafeEqual(Buffer.from(claims.email, 'hex'), emailDigest(email))) return void generic()
