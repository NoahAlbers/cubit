...
router.get('/memberActivity/:memberId', async (req, res, next) => {
  try {
    res.json(await memberActivity(AppDataSource.manager, req.params.memberId));
  } catch (error) {
    next(error);
  }
});
...
router.post('/', async (req, res, next) => {
  try {
    const b = req.body,
      serial = typeof b.serialNumber === 'string' ? b.serialNumber.trim() : '';
    if (
      typeof b.id !== 'string' ||
      !b.id ||
      b.id.length > 36 ||
      ...
      fail('Enter a fob serial number, member and valid status.');
    const saved = await AppDataSource.transaction(async (manager) => {
      ...
      if (before && before.memberId !== b.memberId)
        fail('A fob cannot be moved to another member through this form.', 409);
      if (before && b.expectedStatus !== before.status)
        fail('This fob changed. Reload before editing.', 409);
      const duplicate = await manager
        .createQueryBuilder(MemberKey, 'k')
        .where('UPPER(TRIM(k.serialNumber))=:serial', { serial: serial.toUpperCase() })
        .andWhere('k.id<>:id', { id: before?.id || '' })
        .getOne();
      if (duplicate)
        fail('This fob is already assigned. Review its existing member before adding it.', 409);
      const reason = before
        ? reasonText(b.reason)
        : typeof b.reason === 'string'
          ? b.reason.trim().slice(0, 500)
          : '';
      const after = await manager.save(
        MemberKey,
      ...
      await recordAudit(manager, {
        memberId: b.memberId,
        kind: before ? 'Fob updated' : 'Fob assigned',
        author: req.member!.email,
        entityId: after.id,
        before: snapshot(before, keyFields),
        after: snapshot(after, keyFields),
        reason,
      });
      return after;
    });
...
router.delete('/:keyId', async (req, res, next) => {
  try {
    ...
    await AppDataSource.transaction(async (manager) => {
      ...
      if (req.body.expectedStatus !== key.status || req.body.expectedSerial !== key.serialNumber)
        fail('This fob changed. Reload before removing it.', 409);
      await recordAudit(manager, {
        memberId: key.memberId,
        kind: 'Fob removed',
        author: req.member!.email,
        entityId: key.id,
        before: snapshot(key, keyFields),
        after: null,
        reason,
      });
