...
router.get('/events', async (req, res, next) => {
  try {
    ...
    const query = AppDataSource.getRepository(AccessLog)
    ...
    if (options.result !== 'all')
      query.andWhere('COALESCE(log.accessGranted, 0) = :granted', {
        granted: options.result === 'granted' ? 1 : 0,
      });
    if (options.search)
      query.andWhere(
        "CONCAT_WS(' ', member.firstName, member.lastName, member.email, log.message) LIKE :search ESCAPE '!'",
    ...
    if (options.sort === 'name')
      query.orderBy('member.lastName', options.order).addOrderBy('member.firstName', options.order);
    else
      query.orderBy(
        options.sort === 'result' ? 'log.accessGranted' : 'log.timestamp',
        options.order,
      );
...
router.get('/getAccessLog', async (req, res, next) => {
  try {
    res.json(await AccessLog.getAccessLog());
  } catch (error) {
    next(error);
  }
});
