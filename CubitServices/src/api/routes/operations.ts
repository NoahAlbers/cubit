...
import { StaffNote, ChargeAdjustment, BillingCharge, OperationsSettings, OperationsAudit, AutomationRun, PaymentEvent } from '../../entity/cubitOperations'
...
router.get('/members/:id/notes', route(async (req: any,res: any) => {
  if (!await AppDataSource.manager.findOneBy(Member,{id:req.params.id})) fail('Member not found.',404)
  res.json(await AppDataSource.manager.find(StaffNote,{where:{memberId:req.params.id},order:{createdAt:'DESC',id:'DESC'}}))
}))
router.post('/members/:id/notes', route(async (req: any,res: any) => {
...
router.get('/members/:id/operations', route(async (req: any,res: any) => {
  ...
  res.json({accessHold:member.accessHold,accessHoldReason:member.accessHoldReason,
    history:await AppDataSource.manager.find(OperationsAudit,{where:{memberId:member.id},order:{createdAt:'DESC'},take:50})})
}))
router.post('/members/:id/operations', route(async(req:any,res:any)=>{
...
router.post('/charges/:id/adjustments', route(async(req:any,res:any)=>{
  const credit=Number(req.body.credit), reason=reasonText(req.body.reason), key=requestKey(req.body.requestKey)
  if(!Number.isFinite(credit)||credit<=0||Math.abs(credit*100-Math.round(credit*100))>0.00001) fail('Enter a positive credit with at most two decimals.')
  const result=await AppDataSource.transaction(async manager=>{
    const lookup=await manager.findOneBy(BillingCharge,{id:req.params.id}); if(!lookup)fail('Charge not found.',404)
    ...
    if(duplicate){if(duplicate.chargeId!==lookup.id||Number(duplicate.credit)!==credit)fail('Request ID conflict.',409);return duplicate}
...
router.post('/members/:id/charges', route(async(req:any,res:any)=>{
  res.json(await AppDataSource.transaction(manager => addCharge(manager, req.params.id, req.body, req.member.email)))
}))
router.post('/charges/:id/correct', route(async(req:any,res:any)=>{
  res.json(await AppDataSource.transaction(manager => editCharge(manager, req.params.id, req.body, req.member.email)))
}))
router.get('/automation', route(async(req:any,res:any)=>{
  const [settings,runs,events,audit]=await Promise.all([
    ...
    AppDataSource.manager.find(OperationsAudit,{where:{kind:'Automation settings changed'},order:{createdAt:'DESC'},take:20})])
...
router.post('/automation/run',route(async(req:any,res:any)=>{
...
router.get('/reports',route(async(req:any,res:any)=>{
  const data=await reportData(req.query)
  const {roster,payments,visits,overdue,...dashboard}=data
  res.json(dashboard)
}))
router.get('/reports/:type.csv',route(async(req:any,res:any)=>{
