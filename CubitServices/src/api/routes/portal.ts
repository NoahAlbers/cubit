...
router.get(
  '/documents/:id',
  route(async (req: any, res: any) => sendDocument(req, res, false)),
);
const profileFields = [
  'firstName',
  'lastName',
...
router.get(
  ...
  route(async (req: any, res: any) => {
    ...
    const state = await ensureBilling(member.id),
      asOf = state.ledger.asOf;
    const enabledKeys = await AppDataSource.manager.countBy(MemberKey, {
      memberId: member.id,
      status: 'Active',
    });
    const plans = state.plans.map((p) => ({
      ...
      monthlyRate: Number(p.billingRate ?? p.plan.monthlyCost),
      startDate: day(p.startDate),
      finalBillingDate: p.finalBillingDate || (p.endDate ? day(p.endDate) : null),
    }));
    const payments = state.payments
      .filter((p) => !p.correctedBy && !p.reversalOf && Number(p.amount) !== 0)
      .map((p) => ({
        id: p.id,
    ...
    res.json({
      ...
      status: state.status,
      entryAllowed: state.status === 'Active' && enabledKeys > 0,
      enabledKeys,
      accessMessage: member.accessHold
        ? 'Please contact staff about your membership access.'
        : state.reason,
      plans: plans.map((p) => ({
        ...p,
        current: p.startDate <= asOf && (!p.finalBillingDate || p.finalBillingDate >= asOf),
      })),
      billing: {
        ...state.ledger,
        payments,
        charges: state.ledger.charges.map((c) => ({
          id: c.id,
          dueDate: c.dueDate,
          planName: c.planName,
          amount: c.amount,
...
router.put(
  ...
  route(async (req: any, res: any) => {
    ...
    for (const k of profileFields) {
      if (
        typeof b[k] !== 'string' ||
        b[k].trim().length > (k === 'phone' || k === 'emergencyPhone' ? 50 : 150)
      )
        fail('Enter valid contact details.');
      values[k] = b[k].trim();
    }
    normalizeContact(values, true);
    if (
      localConfig.runtimeMode === 'hosted-demo' &&
      req.member.id === demoMemberId &&
    ...
    await AppDataSource.transaction(async (manager) => {
      ...
      if (values.email !== member.email)
        await manager.save(AccountNotice, {
          memberId: member.id,
      ...
      await recordAudit(manager, {
        memberId: member.id,
        kind: 'Member updated contact details',
        author: member.email,
        actorType: 'member',
        entityId: member.id,
        before: snapshot(member, [...profileFields]),
        after: values,
      });
    });
...
router.post(
  '/waivers/:versionId/start',
  route(async (req: any, res: any) =>
    res.json(await startSigning(req.member, req.params.versionId)),
  ),
);
router.post(
  '/signatures/:id/demo-complete',
  route(async (req: any, res: any) =>
    res.json(await completeDemo(req.member.id, req.params.id, req.body)),
  ),
);
router.post(
  '/signatures/:id/sync',
  route(async (req: any, res: any) => res.json(await syncSigning(req.member.id, req.params.id))),
);
