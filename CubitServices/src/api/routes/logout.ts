...
router.post('/', signedIn, async (req, res, next) => {
  try {
    await AppDataSource.transaction(async manager => {
      ...
      await recordAudit(manager, { memberId: req.member!.id, kind: 'Signed out everywhere',
        author: req.member!.email, actorType: req.member!.role === 'admin' ? 'staff' : 'member',
        entityId: req.member!.id, after: { sessionsRevoked: true } })
    })
    res.status(204).end()
