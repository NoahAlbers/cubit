...
import { billingLedger, day, validDay } from '../../billing/ledger';
...
router.get('/members/:id/billing', async (req, res, next) => {
  try {
    ...
    const { plans, ledger, payments, adjustments, records, status, reason } =
      await new Member().getBilling(req.params.id);
    const history = await AppDataSource.manager.find(BillingChange, {
    ...
    res.json({
      ...
      history,
      adjustments,
      voidedCharges: records.filter((c) => c.voided),
      status,
      accessReason: reason,
    });
  } catch (err) {
...
router.post('/plans/:id/cutoff', async (req, res, next) => {
  try {
    ...
    if (
      !validDay(finalBillingDate) ||
      finalBillingDate < '1900-01-01' ||
      finalBillingDate > '2100-12-31'
    )
    ...
    const result = await AppDataSource.transaction(async (manager) => {
      ...
      if (finalBillingDate < day(plan.startDate))
        throw Object.assign(new Error('Final billing date cannot precede the plan start.'), {
          status: 400,
        });
      const previousDate = plan.finalBillingDate || (plan.endDate ? day(plan.endDate) : null);
      if (expectedDate !== previousDate)
        throw Object.assign(new Error('The plan changed. Refresh and review it again.'), {
          status: 409,
        });
      ...
      const {
        ...
        adjustments,
        ledger: before,
      } = await readBilling(manager, plan.memberId);
      if (
        plans.some(
          (p) =>
            p.id !== plan.id &&
            day(p.startDate) <= finalBillingDate &&
            (!(p.finalBillingDate || p.endDate) ||
              day(p.finalBillingDate || p.endDate) >= day(plan.startDate)),
        )
      )
        throw Object.assign(new Error('This date would overlap another membership plan.'), {
          status: 409,
        });
      ...
      const projected = records.map((c) =>
        c.memberPlanId === plan.id ? { ...c, voided: c.dueDate > finalBillingDate } : c,
      );
      // An extended cutoff can expose anniversaries never posted under the old cutoff.
      const scheduled = billingLedger(
        [{ ...changed, plan: { name: changed.billingName, monthlyCost: changed.billingRate! } }],
        [],
      ).charges;
      for (const c of scheduled)
        if (!projected.some((p) => p.memberPlanId === plan.id && p.dueDate === c.dueDate))
          projected.push({
            id: `preview-${c.dueDate}`,
      ...
      if (preview !== true && previousDate !== finalBillingDate) {
        ...
        await manager.save(
          BillingChange,
          manager.create(BillingChange, {
            memberId: plan.memberId,
            memberPlanId: plan.id,
            previousDate,
            finalBillingDate,
            reason: reason.trim(),
            changedBy: req.member!.email,
          }),
        );
        await recordAudit(manager, {
          memberId: plan.memberId,
          kind: 'Final billing date changed',
          author: req.member!.email,
          entityId: plan.id,
          before: { finalBillingDate: previousDate },
          after: { finalBillingDate },
          reason: reason.trim(),
        });
