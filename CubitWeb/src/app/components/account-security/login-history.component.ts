...
@Component({selector:'app-login-history',standalone:true,imports:[CommonModule],changeDetection:ChangeDetectionStrategy.Eager,
  ...
  @for(row of data.rows;track row.id){<article><div class="history-heading"><strong>{{row.device.browser}} · {{row.device.os}}</strong><time>{{row.createdAt|date:'medium'}}</time></div><p>{{row.device.device}} · {{row.method}}</p><p>IP: {{row.device.ip||'Unavailable'}}</p><p class="muted">{{row.location||'Location unavailable'}}</p></article>}
