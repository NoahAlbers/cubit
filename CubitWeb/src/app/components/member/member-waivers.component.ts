...
<p *ngIf="error" class="error" role="alert">{{error}}</p><ng-container *ngIf="data">
...
export class MemberWaiversComponent implements OnChanges {
  ...
  ngOnChanges(){this.data=null;this.error='';this.http.get('/api/waivers/members/'+this.memberId).subscribe({next:d=>{this.data=d;this.ready.emit();},error:e=>{this.error=e.error?.message||'Unable to load waivers.';this.ready.emit();}});}
}
