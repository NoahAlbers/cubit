...
export class AuditLogComponent implements OnInit,OnDestroy {
  ...
  load(){const version=++this.version;this.loading=true;this.error='';this.expanded='';const params:any={q:this.q,from:this.from,to:this.to,author:this.author,kind:this.kind,actor:this.actor,memberId:this.memberId,sort:this.sort,order:this.order,page:String(this.page),pageSize:String(this.pageSize)};this.http.get<any>('/api/cubit/audit',{params}).subscribe({next:d=>{if(version!==this.version)return;this.data=d;if(this.memberId&&d.member?.name)this.navigation.rememberMember(this.memberId,d.member.name);this.page=d.page;this.loading=false;this.navigation.restoreScroll();},error:e=>{if(version!==this.version)return;this.loading=false;this.error=e.error?.message||'Could not load audit history.';}});}
