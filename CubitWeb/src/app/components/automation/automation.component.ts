...
export class AutomationComponent implements OnInit {
  ...
  ngOnInit(){this.load();}
  load(){this.http.get<any>('/api/cubit/automation').subscribe({next:d=>{this.data=d;this.settings={...d.settings};this.graceEnabled=d.settings.graceDays>0;if(this.graceEnabled)this.graceDays=d.settings.graceDays;},error:e=>this.error=e.error?.message||'Could not load automation.'});}
