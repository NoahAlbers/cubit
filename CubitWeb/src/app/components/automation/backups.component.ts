...
export class BackupsComponent implements OnInit,OnDestroy {
 ...
 prune(){if(window.confirm('Keep the newest '+this.data.settings.localKeep+' local backups and remove older local copies? Off-server copies will not be changed.'))this.queue('prune');}
 ...
 queue(kind:string,snapshot?:string){if(this.busy)return;this.busy=true;this.error='';this.message='';this.http.post('/api/backups/jobs',{kind,...(snapshot?{snapshot}:{}),...(kind==='prune'?{revision:this.data.revision}:{})}).subscribe({next:()=>{this.busy=false;this.message=kind==='backup'?'Backup queued. The worker will start it shortly.':kind==='prune'?'Local retention cleanup queued.':'Recovery test queued. Your working database will stay unchanged.';this.load();},error:e=>{this.busy=false;this.error=e.error?.message||'Could not queue the operation.';}});}
}
