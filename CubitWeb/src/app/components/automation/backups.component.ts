...
export class BackupsComponent implements OnInit,OnDestroy {
 ...
 get remoteHealthy(){const r=this.data?.runtime,b=r?.vault?.backup;return this.remoteFresh&&this.auditHealthy&&r.remoteSync?.ok!==false&&this.data?.settings?.offsiteEnabled&&b?.snapshots?.length>0&&b?.lastRestore?.ok&&!b.error&&this.usage(r.vault?.storage?.totalBytes,r.vault?.storage?.freeBytes)<80&&(!r.vault?.memory||this.usage(r.vault.memory.totalBytes,r.vault.memory.availableBytes)<90);}
