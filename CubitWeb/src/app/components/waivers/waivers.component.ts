...
export class WaiversComponent implements OnInit {
  ...
  constructor(private drafts:DraftGuard,private http:HttpClient, public navigation:ListNavigationService, private route:ActivatedRoute, private router:Router){}
  ...
  load(){this.http.get<any>('/api/waivers').subscribe({next:d=>{this.data=d;this.navigation.restoreScroll();},error:e=>this.error=e.error?.message||'Unable to load waivers.'});}
  ...
  async edit(w?:any){if(this.hasUnsavedChanges()&&!await this.drafts.confirmDiscard())return;this.tab='templates';this.error='';this.saved='';this.editing=w||null;this.confirmed=false;this.form={name:w?.name||'',description:w?.version.description||'',required:w?.required??true,
    provider:w?.version.provider||'demo',demoText:w?.version.demoText||'LOCAL DEMONSTRATION — NOT A LEGAL WAIVER\n\nReplace this text with a sample document for testing.',
