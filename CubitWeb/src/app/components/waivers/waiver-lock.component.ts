...
@Component({selector:'app-waiver-lock',template:`
<section class="waiver-lock"><span class="badge">Work in progress</span><h2>Waivers preview</h2><p>This feature is still being built. Enter the preview password to explore it.</p><form (ngSubmit)="unlock()"><label>Preview password<input type="password" name="previewPassword" [(ngModel)]="password" autocomplete="off" required [disabled]="busy"></label><p *ngIf="error" class="error" role="alert">{{error}}</p><button class="primary" type="submit" [disabled]="busy||!password">{{busy?'Unlocking…':'Unlock preview'}}</button></form></section>
`})
