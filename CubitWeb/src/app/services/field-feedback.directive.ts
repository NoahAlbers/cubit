...
export class FieldFeedbackDirective implements DoCheck,OnDestroy {
  ...
  ngDoCheck(){
    ...
    if(!this.message){this.message=this.renderer.createElement('small');this.renderer.setAttribute(this.message,'id',this.id);this.renderer.addClass(this.message,'field-error');const parent=this.el.nativeElement.parentNode;this.renderer.appendChild(parent.classList.contains('copy-input-wrap')?parent.parentNode:parent,this.message);const described=this.el.nativeElement.getAttribute('aria-describedby');this.renderer.setAttribute(this.el.nativeElement,'aria-describedby',[described,this.id].filter(Boolean).join(' '));}
