...
export class FieldFeedbackDirective implements DoCheck,OnDestroy {
  ...
  ngDoCheck(){
    ...
    const text=c?.touched&&e?(e.required?'This field is required.':e.email?'Enter a valid email address.':e.min?'Enter a value of at least '+e.min.min+'.':e.max?'Enter a value no greater than '+e.max.max+'.':e.maxlength?'This value is too long.':'Check this value.') : '';
