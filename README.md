...
- Existing billing and note history is preserved once during migration. Preserved rows are identified, and missing historical values are not invented. Staff actions are the default; system/member activity can also be included. Date filters and displayed audit times use the organization time zone; storage remains UTC.
- When the dedicated backup server is connected, audit events enter a transactional outbox and are archived every 15 seconds. Staff read the protected remote archive; an outage shows a labelled local fallback. The CRM cannot update or delete accepted archive records. Synthetic demo/local workspaces never contact that archive.
...
**Settings & Automation → Backups & recovery** controls manual, daily, weekly, or monthly encrypted backups, local retention, and isolated restore tests. The inventory shows local and connected off-server copies, storage capacity, audit counts and independent recovery status. Only Administration can change schedules or prune local copies; Staff Users can inspect, create and test backups. Snapshots include both Cubit databases, uploaded waivers, DocuSeal files and SQLite, and recovery requirements. Private service secrets are held separately in approved operator custody.

The dedicated backup VPS accepts encrypted snapshots and append-only audit records over HTTPS. The CRM cannot delete or overwrite retained objects, use backup-server SSH, change retention, or access its root-only recovery configuration. Failed transfers retry; cleanup waits for off-server copying when enabled. The backup operator controls at least 30 days of receipt-based retention and keeps the last verified recovery point until another passes. Audits have no automatic deletion. Independent weekly tests restore into a networkless database and verify retained documents. See [vault setup and limits](deploy/vault/README.md) and [the recovery runbook](deploy/BACKUPS.md). A full replacement-application rehearsal and independent monitoring remain launch requirements.
...
Organization Management also selects the display currency (default USD). All money labels, symbols, chart tooltips and CSV headings follow it. This is a single-currency ledger using two-decimal amounts: changing the setting relabels existing values without currency conversion. Offline test events must match the selected currency when a currency is supplied. Currency-aware live imports remain disabled and require separate validation before launch.

Administrators can review or reopen failed backup/recovery operations with a reason. The original failure/result remains visible in paginated operation history and each review is audited. Reviewed failures stop contributing to the recent-failure health warning; current worker, storage, HTTPS and recovery checks remain independent.
