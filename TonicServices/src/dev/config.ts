...
if (process.env.CUBIT_MODE !== 'hosted-review') {
...
export const localConfig = process.env.CUBIT_MODE === 'hosted-review'
  ? readHostedReviewConfig(process.env) : readLocalConfig(process.env)
// The captured Tonic server uses UTC calendar days. Preserve its billing dates.
if (localConfig.dataMode === 'imported') process.env.TZ = 'UTC'
