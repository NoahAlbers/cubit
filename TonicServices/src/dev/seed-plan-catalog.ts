...
export async function seedPlanCatalog() {
  ...
  await AppDataSource.transaction(async manager => {
    const options = [
      ['Standard Membership', 60], ['Student Membership (18–28)', 30],
      ['Standard + 1 Key', 90], ['Standard + 2 Keys', 120], ['Standard + 3 Keys', 150],
    ] as const
    for (const [index, [name, monthlyCost]] of options.entries()) {
      await manager.save(Plan, manager.create(Plan, { id: id(index + 1), name, monthlyCost, available: true }))
    }
    for (const legacyId of ['10000000-0000-4000-8000-000000000001',
