...
export interface BillingPlan {
  id: string; startDate: Date | string; endDate?: Date | string | null
  finalBillingDate?: string | null
  plan: { name: string; monthlyCost: number | string }
}
export interface Payment { amount: Number | string; transactionDate: Date | string }
...
export function billingLedger(plans: BillingPlan[], payments: Payment[], asOf = day(new Date())) {
  ...
  const charges: { planId: string; planName: string; dueDate: string; amount: number; outstanding: number }[] = []
  for (const plan of plans) {
    const start = day(plan.startDate)
    const stop = [asOf, plan.endDate && day(plan.endDate), plan.finalBillingDate].filter(Boolean).sort()[0] as string
    ...
    for (let month = 0; ; month++) {
      ...
      charges.push({ planId: plan.id, planName: plan.plan.name, dueDate, amount: cents, outstanding: cents })
    }
  }
  charges.sort((a, b) => a.dueDate.localeCompare(b.dueDate) || a.planId.localeCompare(b.planId))
  const paid = payments.filter(p => day(p.transactionDate) <= asOf)
  ...
  return {
    asOf, totalCharges: total / 100, totalPaid: paid / 100,
    balance: (total - paid) / 100, pastDue: overdue / 100,
    credit: Math.max(0, paid - total) / 100, oldestUnpaidDate,
...
export function membershipStatus(plans: BillingPlan[], balance: number, asOf = day(new Date())) {
  const current = plans.filter(p => day(p.startDate) <= asOf &&
    (!p.endDate || day(p.endDate) >= asOf) && (!p.finalBillingDate || p.finalBillingDate >= asOf))
  ...
  return plans.some(p => (p.finalBillingDate && p.finalBillingDate < asOf) ||
    (p.endDate && day(p.endDate) < asOf)) ? 'Canceled' : 'Inactive'
}
